...
import {SendPengaduan, UploadImage} from "@/utils/api/features/pengaduan_utils";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        const {ktp, title, ket, alamat, photo, auth} = req.body;

        if (!ktp || !title || !ket || !alamat || !photo || !auth) {
        ...
        const dbQuery = await SendPengaduan(ktp, title, ket, alamat, uploadImage.file!);
        ...
        return res.status(StatusCodes.CREATED).json({status: "success", message: 'Berhasil mengirim pengaduan'})
