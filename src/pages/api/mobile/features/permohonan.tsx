...
import {SubmitPermohonan, UploadPDF} from "@/utils/api/features/permohonan_utils";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        const {ktp, title, ket, alamat, document, auth} = req.body;

        if (!ktp || !title || !ket || !alamat || !document || !auth) {
        ...
        const dbQuery = await SubmitPermohonan(ktp, title, ket, alamat, UploadPdf.file!)
        ...
        return res.status(StatusCodes.CREATED).json({status: "success", message: 'Berhasil mengirim permohonan'})
