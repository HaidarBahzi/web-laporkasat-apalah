...
import { GetDashboardPegawai } from "@/utils/server/dashboard/dashboard";
...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchDetailDashboard() {
      ...
      const statistic = await GetDashboardPegawai(session.idUser!);
      ...
      <div className="form-control gap-10">
        <div className="flex gap-5 z-0">
          <StatsMenu
            ...
            title={"Pengaduan"}
            link={"/user/menu_layanan/pengaduan"}
          />
          ...
          <StatsMenu
            ...
            icon={<FaHandsHelping />}
            title={"Permohonan"}
            link={"/user/menu_layanan/permohonan_bantuan"}
          />
        </div>
