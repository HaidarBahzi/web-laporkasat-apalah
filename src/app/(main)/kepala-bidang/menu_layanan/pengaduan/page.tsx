...
import {
  GetAllPengaduanBidang,
  GetAllPengaduanKepalaBidang,
  GetAllPengaduanKepalaBidangDate,
...
import { CheckPelanggaran } from "@/utils/server/pelanggaran/pelanggaran";
...
export default function Page() {
  ...
  const [pelanggaran, setPelanggaran] = useState<{ [key: string]: number }>({});
  ...
  async function FetchAllData() {
    try {
      ...
      const callAllPengaduan = await GetAllPengaduanKepalaBidang(
        session.bidangId!
      );
      ...
      for (const item of callAllPengaduan!) {
        const result = await CheckPelanggaran(item.laporan_id);
        violationsData[item.laporan_id] = result;
      }
      setPelanggaran(violationsData);
    } catch (e) {
      console.error(e);
    }
  }
    ...
    <section className="container mx-auto px-16">
      <MenuBreadCrumbs
        ...
        titleLinkArray={[
          ...
          "/kepala-bidang/menu_layanan/pengaduan",
        ]}
      ...
      <MenuContainer>
        ...
        <ModalSearchDate
          ...
          onSubmit={async (dateFirst: Date, dateSec: Date) => {
            ...
            const callAllPengaduan = await GetAllPengaduanKepalaBidangDate(
              session.bidangId!,
              dateFirst,
              dateSec
            );
                      ...
                      <td>
                        <div className={"flex gap-2 justify-center"}>
                          {pelanggaran[value.laporan_id] === 0 ? (
                            <ButtonActionLinkMenu
                              link={`/kepala-bidang/menu_layanan/pengaduan/detaildata/${value.laporan_id}`}
                              btnType={"btn-warning"}
                          ...
                          <ButtonActionFunctionMenu
                            btnFunction={() =>
                              PrintData(
                                ...
                                `http://103.30.180.221/web-laporkasat-apalah/assets/foto-pengaduan/${value.laporan_document}`
                              )
                            }
                            btnType={"btn-info"}
